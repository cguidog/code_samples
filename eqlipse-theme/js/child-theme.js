...
(function (global, factory) {
	...
	*/
	...
	(function ($) {
	  ...
	  const options = {
	    root: null,
	    // Sets the observing viewport to the browser's viewport.
	    rootMargin: '0px',
	    threshold: 0.3
	  };
	...
	(function ($) {
	  ...
	  /**
	   * Use jQuery to select all 'descriptive-list-item' elements with the class and start observing 
	   * each of them to detect when they intersect with the viewport.
	   */
	  $('.descriptive-list-item').each(function () {
	    observer.observe(this);
	  });
	})(jQuery);
	...
	(function ($) {
	  ...
	  if ($('.page-navigation').length > 0) {
	    //Handles Page Navigation element
	    ...
	    $(window).on('scroll', function () {
	      if ($(window).scrollTop() > top - height) {
	        //Reveals header page navigation.
	        $('.page-navigation').addClass('visible');
	      }
	      if ($('.page-navigation').hasClass('visible') && $(window).scrollTop() < top - height) {
	        //Hides header page navigation.
	    ...
	    $('.page-navigation .page-navigation_link').on('click', function (e) {
	      targetScroll(e, this);
	    });
	  }
	  $(window).on('load', function () {
	    //Overrides native target scroll.
	...
	(function ($) {
	  ...
	  const callback = (entries, observer) => {
	    entries.forEach(entry => {
	      if (entry.isIntersecting) {
	        ...
	        $(previous).addClass('in-viewport');
	        setTimeout(function () {
	          $(previous).find('.split-text').each(function (index, element) {
	            setTimeout(function () {
	              $(element).addClass('visible');
	            }, 25 * (index + 1));
	          });
	  ...
	  $('.timeline-load-more').on('click', function () {
	    ...
	    $(events).last().addClass('last-event');
	    $(this).remove();
	  });
	})(jQuery);
	...
	(function ($) {
	  ...
	  function initMap($el) {
	    ...
	    // Setup map arguments
	    const mapArgs = {
	      mapId: $el.data('map_id'),
	      // map_id data attribute of the map container
	      zoom: $el.data('zoom') || 16,
	      // zoom level, default to 16 if not set
	      // Setting up map options
	      mapTypeId: google.maps.MapTypeId.ROADMAP,
	      // initial map type is roadmap
	  ...
	  function initMarker($marker, map) {
	    ...
	    let marker = new google.maps.Marker({
	      ...
	      map: map,
	      // Map instance on which to place the marker
	      icon: inactiveIcon,
	...
	*/
	...
	var splittingExports = {};
	var splitting = {
	  get exports(){ return splittingExports; },
	  set exports(v){ splittingExports = v; },
	};
	...
	var Splitting = splittingExports;
	...
	const scroll = () => {
	  fx1Titles.forEach(title => {
	    gsapWithCSS.fromTo(title.querySelectorAll('.char'), {
	      'will-change': 'opacity',
	      opacity: 0.1
	    }, {
	      ease: 'none',
	  ...
	  fx2Titles.forEach(title => {
	    gsapWithCSS.fromTo(title.querySelectorAll('.word'), {
	      'will-change': 'opacity',
	      opacity: 0.1
	    }, {
	      ease: 'none',
	...
	(function ($) {
	  ...
	  $('.post-slider').on('mouseenter', function () {
	    $(this).on('wheel.scrollHorizontally', function (e) {
	      ...
	      if (e.originalEvent.deltaY > 0 && currentScrollLeft < maxScrollLeft || e.originalEvent.deltaY < 0 && currentScrollLeft > 0) {
